    ...
    while link_idx < len(lines):
        ...
        for data in pages_data:
            ...
            # print(answer)
                ...
                with open('not_saved_links.txt', 'a', encoding='utf-8') as not_saved_file:
                    not_saved_file.writelines(f'{list(data.keys())[0]}')
                    print(f'[BAD URL] --- {list(data.keys())[0]}')
        with open('links.txt', 'w', encoding='utf-8') as f2:
            f2.writelines(lines[link_idx + 10: len(lines)])
            f2.close()
        with open('links.txt', 'r', encoding='utf-8') as f3:
            lines = f3.readlines()
        pages_data.clear()
        for url in bad_urls:
            with open('not_saved_links.txt', 'a', encoding='utf-8') as fff:
